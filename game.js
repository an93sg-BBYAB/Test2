...
class GameScene extends Phaser.Scene {
    constructor() {
        ...
        this.isInitialDrawComplete = false; // [신규] 초기 redraw 완료 여부
    }
    ...
    create() {
        ...
        this.time.delayedCall(200, () => { 
             ...
             if (uiScene && this.scene.isActive('UIScene')) { // UIScene 존재 및 활성 확인
    ...
    redraw(gameSize) {
        ...
        if (!this.hero && this.pathCoordsWithOffset.length > 0) { 
            ...
            if (this.hero.body) {
                ...
                // 전투 후 body가 없을 수 있으므로 다시 활성화 시도
        ...
        this.isInitialDrawComplete = true; // [신규] 초기 그리기 완료 표시
    ...
    generateRandomLoop() {
        ...
        for(const segment of segments) {
            ...
            if (len > 3 && Math.random() < 0.5 && totalTiles + len + 4 < maxTiles) { 
                ...
                for(let i=0; i<detourLength; i++) {
                    cx += dx1; cy += dy1;
                    if (cx<1 || cx>=this.GRID_WIDTH-1 || cy<1 || cy>=this.GRID_HEIGHT-1 || (this.grid[cy] && this.grid[cy][cx] !== TILE_TYPE_EMPTY)) { possible = false; break; } // grid[cy] 확인 추가
            ...
            totalTiles += segmentPathPoints.length -1;
             ...
             segmentPathPoints.forEach(p => {
                 if (!this.grid[p.y]) this.grid[p.y] = []; // 행 없으면 생성
                 if(this.grid[p.y][p.x] === TILE_TYPE_EMPTY) this.grid[p.y][p.x] = TILE_TYPE_PATH; // 비어있을 때만 경로 표시 (Detour 경로 유지)
             });
        }
        
        // 최종 경로 유효성 검사 및 Fallback
        if (finalPath.length < 10 || !finalPath[0] || finalPath[0].x !== startX || finalPath[0].y !== startY) {
            ...
            this.generateDefaultLoop(); 
             ...
             // 시작점으로 닫기 (이 점은 이미 grid에 PATH로 표시됨)
        ...
        if(this.pathCoords.length > 10) { 
            ...
            while(pathIndices.length > 0 && (count2 < 2 || count3 < 3 || count5 < 1)) {
                 ...
                 if (index === undefined) break; // 인덱스 없으면 종료
        ...
        } else if (this.pathCoords.length > 0) { // Default 루프 생성 시에도 특수 타일 지정
             console.log("Assigning special tiles for default loop.");
             // Default 루프에 대한 특수 타일 지정 (generateDefaultLoop 내 로직 이동)
              const pathIndices = Array.from(this.pathCoords.keys()); 
              pathIndices.shift(); 
              Phaser.Utils.Array.Shuffle(pathIndices); 
              if(pathIndices.length > 0) {
                 const idx2 = pathIndices.pop();
                 if(idx2 !== undefined && this.pathCoords[idx2]) this.grid[this.pathCoords[idx2].y][this.pathCoords[idx2].x] = TILE_TYPE_ENEMY2;
              }
              if(pathIndices.length > 0) {
                 const idx3 = pathIndices.pop();
                  if(idx3 !== undefined && this.pathCoords[idx3]) this.grid[this.pathCoords[idx3].y][this.pathCoords[idx3].x] = TILE_TYPE_ENEMY3;
              }
               if(pathIndices.length > 0) {
                 const idx5 = pathIndices.pop();
                  if(idx5 !== undefined && this.pathCoords[idx5]) this.grid[this.pathCoords[idx5].y][this.pathCoords[idx5].x] = TILE_TYPE_ENEMY5;
              }
        }
        
        console.log("Generated loop length:", this.pathCoords.length);
        console.log("Grid dimensions:", this.grid.length, this.grid[0]?.length);
    ...
    generateDefaultLoop() {
        ...
        if (this.pathCoords.length > 0) { // 시작점으로 닫기
            this.pathCoords.push(this.pathCoords[0]);
        }
    }
    ...
    updatePathCoordsWithOffset() {
        ...
        this.pathCoordsWithOffset = this.pathCoords.map(coord => {
         ...
         if (this.pathCoordsWithOffset.length === 0 && this.pathCoords.length > 0) { // 필터링 후 비었는지 확인
    ...
    onCombatComplete(data) {
        ...
        // hero가 destroy된 경우를 처리
        if (!this.hero) return; 

        this.hero.hp = data.heroHp;
        // UIScene 활성 확인 후 이벤트 전송
        const uiScene = this.scene.get('UIScene');
        ...
        if (this.hero.hp <= 0) {
            this.hero.destroy(); // 영웅 객체 제거
            this.hero = null; // 참조 제거
             // 게임 오버 텍스트는 redraw 이후에도 남아있어야 함
             ...
             // 다른 그래픽 요소들이 redraw로 지워질 때 같이 지워지지 않도록 그룹에 추가하지 않음
        } else {
            this.scene.resume();
...
class CombatScene extends Phaser.Scene {
    ...
    endCombat(loot) {
        ...
        if (gameScene && gameScene.events) { // GameScene 존재 및 이벤트 시스템 확인
...
class UIScene extends Phaser.Scene {
    ...
    create() {
        ...
        if (gameScene && gameScene.events) {
            ...
            gameScene.events.on('updateHeroHP', this.updateHeroHP, this);
             ...
             // GameScene이 준비되면 리스너를 다시 등록할 필요가 있을 수 있음
             // 예를 들어, GameScene의 create 끝에서 UIScene에 'ready' 이벤트를 보내는 방식
        }
    ...
    redraw(gameSize) {
        ...
        if (gameSceneRef && gameSceneRef.heroData) { // heroData 확인
            initialHp = gameSceneRef.heroData.hp;
            initialMaxHp = gameSceneRef.heroData.maxHp;
        } 
        if (gameSceneRef && gameSceneRef.hero) { // hero 확인
    ...
    onSlotClick(slot) {
        ...
        if (this.selectedItemIndex !== null) {
            ...
            if (typeof slotKey === 'number' && slotKey < this.inventory.length && this.inventory[slotKey]) { // 인덱스 범위 확인 추가
                this.selectedItemIndex = slotKey;
                this.selectedHighlight.visible = true;
                // 하이라이트 객체가 생성되었는지 확인 후 그리기
    ...
    refreshInventory() {
        ...
        this.inventory.forEach((itemKey, index) => {
            if (itemKey) {
                // inventorySlots 배열 길이 및 슬롯 객체 확인
    ...
    equipItem(itemKey, slotKey) {
        const slot = this.equipSlots[slotKey];
         // 슬롯 존재 및 setData 함수 확인
        if (slot && typeof slot.setData === 'function') {
             slot.setData('item', itemKey);
        } else {
             console.error(`Equip slot ${slotKey} not found
